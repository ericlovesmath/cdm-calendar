...
with requests.get(cal) as r:
    with open(os.path.expanduser("~/Desktop/Bash/cal.pdf"), "wb") as f:
        f.write(r.content)

with open(os.path.expanduser("~/Desktop/Bash/cal.pdf"), "rb") as f:
...
endWeekMonth = int( (date + timedelta(days=(5 - weekDayNum))).strftime('%m') )
endWeekDay = int( (date + timedelta(days=(5 - weekDayNum))).strftime('%d') )
...
print("      " + pdf[0]
    ...
    .replace(dayOfWeek,f"\033[0;36m{dayOfWeek}\033[0m")    # Color the day of the week cyan
    )
